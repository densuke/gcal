...
| **v0.7.1** | 設定ファイルの多層読み込み（ホーム/XDG/OS標準/カレント/指定パス）と可視化機能（`--show-config`, `--verbose`）追加 |
...
| 4 | カレント | `./.gcal.toml` |
| 5 (高) | 指定パス | `--config <PATH>` で指定されたファイル |
...
| フェーズ | 内容 | 優先度 |
|---------|------|--------|
| 1 | `--prompt/-p` を `add`/`update` に追加（`--ai` のエイリアス化） | 高 |
| 2 | `delete -p` の実装（イベント特定フロー） | 高 |
| 3 | `EventSummary` への `location` 追加 | 中 |
| 4 | `gcal events -p` の統合ディスパッチ実装 | 中 |
| 5 | テスト整備（モック LLM + 候補選択フロー） | 高 |
