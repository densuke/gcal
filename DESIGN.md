...
| **v0.6.0** *(開発中)* | 複数カレンダー横断 events 表示（`--calendars`、config デフォルト複数カレンダー） |
...
`gcal events` で複数カレンダーのイベントを時間順に混在表示する。
...
`[events]` セクションはオプション（省略時は空 → `primary` にフォールバック）。
...
| Step | ファイル | テスト内容 |
|------|---------|-----------|
| 1 | `config.rs` | `EventsConfig` デシリアライズ、デフォルト空、`[events]` セクション省略互換 |
| 2 | `config.rs` | `resolve_event_calendars()` の全分岐（--calendars / --calendar / デフォルト / primary） |
| 3 | `app.rs` | 単一カレンダーの後退テスト、複数カレンダー取得+ソートテスト（wiremock） |
| 4 | `cli.rs` | `--calendars` と `--calendar` の排他バリデーション |
| 5 | `main.rs` | Events ディスパッチ接続 |
